...
if __name__ == "__main__":
    ...
    # load NLG & NLU model
    nlg_model_path_default = './deep_dialog/models/nlg/lstm_tanh_relu_[1468202263.38]_2_0.610.p'
    nlu_model_path_default = './deep_dialog/models/nlu/lstm_[1468447442.91]_39_80_0.921.p'
    parser.add_argument('--nlg_model_path', dest='nlg_model_path', type=str, default=nlg_model_path_default, help='path to model file')
    parser.add_argument('--nlu_model_path', dest='nlu_model_path', type=str, default=nlu_model_path_default, help='path to the NLU model file')
...
if usr == 0:# real user
    ...
    user_sim = RealSelectUser(movie_dictionary, act_set, slot_set, goal_set, usersim_params)
elif usr == 3:
    user_sim = RuleSimulator2(movie_dictionary, act_set, slot_set, goal_set, usersim_params)
...
best_model['model'] = agent # copy.deepcopy(agent)
...
def save_model(path, agt, success_rate, agent, best_epoch, cur_epoch):
    filename = 'tf_agt_%s_%s_%s_%.5f.p' % (agt, best_epoch, cur_epoch, success_rate)
    filepath = os.path.join(path, filename)
    agent.dqn.saver.save(agent.dqn.tf_sess, filepath, global_step = cur_epoch)

def save_numpy_model(path, agt, success_rate, agent, best_epoch, cur_epoch):
...
def run_episodes(count, status):
    ...
    for episode in xrange(count):
        ...
        if agt == 9 and params['trained_model_path'] == None:
            ...
            if simulation_res['success_rate'] > best_res['success_rate']:
                agent.dqn.copy_q_net_to_best_net() # Corresponding to: best_model['model'] = copy.deepcopy(agent)
                best_res['success_rate'] = simulation_res['success_rate']
                best_res['ave_reward'] = simulation_res['ave_reward']
                best_res['ave_turns'] = simulation_res['ave_turns']
                best_res['epoch'] = episode
                # else:
                #    agent.dqn.copy_best_net_to_q_net()

            agent.dqn.copy_q_net_to_target() # Corresponding to: agent.clone_dqn = copy.deepcopy(agent.dqn)
