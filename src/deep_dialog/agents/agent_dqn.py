...
#        self.clone_dqn = copy.deepcopy(self.dqn)
        ...
        opt_with_hyper_params.optimizer = tf.train.RMSPropOptimizer(learning_rate=opt_with_hyper_params.learning_rate, decay=opt_with_hyper_params.decay_rate, momentum=opt_with_hyper_params.momentum, epsilon=opt_with_hyper_params.smooth_eps) 
        self.dqn = AgentQNet(agent_name, opt_with_hyper_params, self.state_dimension, self.hidden_size, self.num_actions)
        # self.tf_saver = tf.train.Saver()

        self.cur_bellman_err = 0
        self.bot_type = "movie"
        self.limit_experience_replay_pool_size = False
