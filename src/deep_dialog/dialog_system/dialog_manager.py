...
class DialogManager:
    ...
    def initialize_episode(self):
        ...
        # print "Turn -2 usr: Check in please."
        # print "Turn -1 sys: Do you have a reservation? Yes or no?"
    ...
    def next_turn(self, record_training_data=True):
        ...
        # 現在の対話履歴から，もっとも良いと思われるエージェントの応答を決定する
        ...
        # pprint(self.agent_action)
        # print("----------")
        ########################################################################
        #   CALL USER TO TAKE HER TURN
        ########################################################################
        # 対話履歴より最新の対話(直前にエージェントが行った発話)を取得
        self.sys_action = self.state_tracker.dialog_history_dictionaries()[-1]
        # システムの応答内容を意図通りにユーザが理解できたとして，ユーザにシステム発話を渡す
        ...
        ########################################################################
        # 対話がまだ終了しない場合，ユーザの対話でstate_trackerをアップデート
        if self.episode_over != True:
            self.state_tracker.update(user_action=self.user_action)
            self.print_function(user_action=self.user_action)
        # pprint(self.user_action)
        # print("----------")
        ########################################################################
        #  Inform agent of the outcome for this timestep (s_t, a_t, r, s_{t+1}, episode_over)
        ########################################################################
        # 学習モードで実行していたら，環境からのフィードバックを保存し，将来の訓練データとする
    ...
    def reward_function(self, dialog_status, normalize_reward=False):
        ...
        if dialog_status == dialog_config.FAILED_DIALOG:
            ...
            reward = -1
        if normalize_reward:
            return reward/float(2.0*self.user.max_turn)
        else:
            return reward

    def reward_function_without_penalty(self, dialog_status, normalize_reward=False):
