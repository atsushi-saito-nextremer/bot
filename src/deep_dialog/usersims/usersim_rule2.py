...
from deep_dialog import reservation_flag as reservation


"""
rest_slot : ユーザが尋ねたいと思ってるスロット．goalに含まれるrequest_slotが入ってる
"""

class RuleSimulator2(UserSimulator):
    ...
    def initialize_episode(self):
        ...
        self.goal = dict(zip(map(lambda s: s.replace("\r",""), self.goal.keys()), self.goal.values()))
        ...
        # 最初はreservation_flagをオフにする
        reservation.set_flag(False)
    ...
    def _sample_action(self):
        ...
        # Hello, My name is xxみたいにして始める．
        if len(self.goal['inform_slots']) > 0:
            # # 1つだけ通知する情報を選択
            # known_slot = "reservation"
            # self.state['inform_slots'][known_slot] = self.goal['inform_slots'][known_slot]
            #
            # # reservationフラグをセット
            # reservation.set_flag(True if u'Yes' in self.goal['inform_slots'][known_slot] else False)
            #
            # if 'reservation' in self.goal['inform_slots'].keys():  # 'reservation' must appear in the first user turn
            #     self.state['inform_slots']['reservation'] = self.goal['inform_slots']['reservation']

            for slot in self.goal['inform_slots'].keys():
                # if known_slot == slot or slot == 'reservation': continue
        ...
        # request_slotsがなければinformタイプの発話(これは起こらない)
        if len(self.state['request_slots']) == 0:
            self.state['diaact'] = 'inform'

        # thanksかclosingタイプの発話なら，対話を終了する(これも起こらない)
    ...
    # slot_err_probabilityがrun.pyの引数に渡されてればランダムにスロットを崩壊させる
    ...
    # 応答を確認された場合，引き続き要求を伝えるか，対話を終了(チケットを予約確定)するか決定
    ...
    # inform_slotをラップする．答えを確認しないと先に進まない，前回に自分が話したスロット値であれば通常通りのslotを返す．
    def response_inform_or_confirm_question(self, system_action):
        """ Response for Confirm_Answer (System Action) """
    ...
    def response_thanks(self, system_action):
        ...
        # request_slot，rest_slotからticketを削除
        ...
        # Thanksタイプなのにスロットが残っていたら対話失敗フラグを立てる
        if len(request_slot_set) > 0 or len(rest_slot_set) > 0:
            self.dialog_status = dialog_config.FAILED_DIALOG

        # ゴールに設定された内容と，エージェントが受け付けた内容が違っていた場合
        # 対話履歴にマッチしない値が含まれた場合は対話失敗フラグを立てる
        for info_slot in self.state['history_slots'].keys():
            ...
            if info_slot in self.goal['inform_slots'].keys():
                if self.state['history_slots'][info_slot] != self.goal['inform_slots'][info_slot]:
                    self.dialog_status = dialog_config.FAILED_DIALOG
        # ticketがエージェントのinform_slotsに含まれるが，マッチしない値だったら対話失敗？
    ...
    def response_request(self, system_action):
        ...
        if len(system_action['request_slots'].keys()) > 0:
            ...
            if slot in self.goal[
                ...
                self.state['diaact'] = "inform"
                # 今回伝える情報はrest_slots，request_slotsから削除する
            ...
            # 要求されたスロットがゴール・対話履歴に含まれるがrest_slotsに含まれない場合(システムが正しくユーザの要求を理解できていない場合)
            ...
            # システムに要求されたスロットがユーザにとっても未知の場合
                ...
                # この場合は応えられないので，スロットを不明にし，requestタイプの発話を返す
    ...
    def response_inform(self, system_action):
        ...
        # inform_slotsにtaskcompleteが含まれるなら終了処理
        if 'taskcomplete' in system_action[
            ...
            for slot in self.goal['inform_slots'].keys():
                # エージェントが通知した情報が，ユーザの要求と異なった場合，denyタイプを返す
                ...
                if slot not in system_action['inform_slots'].keys() or (
                    ...
                    self.constraint_check = dialog_config.CONSTRAINT_CHECK_FAILURE
                    break
        # taskcompleteが含まれないinformタイプへの返答
        else:
            for slot in system_action['inform_slots'].keys():
                self.state['history_slots'][slot] = system_action['inform_slots'][slot]
                # 通知されたslotが，ゴールに含まれる場合
                if slot in self.goal['inform_slots'].keys():
                    # 通知されたslotがゴールと合致した場合
                    if system_action['inform_slots'][slot] == self.goal['inform_slots'][slot]:
                        if slot in self.state['rest_slots']: self.state['rest_slots'].remove(slot)
                        # まだrequest_slotsが残っているなら，引き続きエージェントにrequestする(requestタイプで返答)
                        if len(self.state['request_slots']) > 0:
                            self.state['diaact'] = "request"
                        # request_slotsはないが，rest_slotsは残っている場合，
                ...
                # エージェントが通知したslotが，ゴールに含まれない場合
                    ...
                    # rest_slotsが存在する場合
                        ...
                        # チケットを除いてもrest_slots_setがあった場合
                        if len(rest_slot_set) > 0:
                            # rest_slot_setから1つ通知するスロットを決定する
                            inform_slot = random.choice(rest_slot_set)  # self.state['rest_slots']
                            if inform_slot in self.goal['inform_slots'].keys():
                                self.state['inform_slots'][inform_slot] = self.goal['inform_slots'][inform_slot]
                                self.state['diaact'] = "inform"
                                ...
                                self.state['diaact'] = "request"
                        # チケット以外rest_slotが存在しなかった場合
                        else:
                            # チケットを要求するrequestタイプの発話を返す
                            self.state['request_slots']['ticket'] = 'UNK'
                            self.state['diaact'] = "request"
                    else:
