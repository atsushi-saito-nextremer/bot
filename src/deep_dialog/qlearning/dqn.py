...
class QNet(object):
    def __init__(self, tf_var_scope_head, input_size, hidden_size, output_size):
        ...
        with tf.variable_scope(tf_var_scope_head + self._component_names[1],  reuse=tf.AUTO_REUSE, initializer=tf.initializers.random_uniform):    
        ...
        with tf.variable_scope(tf_var_scope_head + self._component_names[3],  reuse=tf.AUTO_REUSE, initializer=tf.initializers.random_uniform):    
    ...
    def build_graph_q_and_t(self):
        ...
        t_h2 = tf.matmul(q_h1, self._qvars["h2"]) + self._qvars["b2"]
...
class AgentQNet(QNet):
    ...
    def calc_forward(self, batched_inputs):
        feed_dict = {self.ph_inputs_q_net:batched_inputs}
        return self._sess.run([self.q_out_op, self.t_out_op], feed_dict=feed_dict)

    def _set_loss_op(self):
        self._loss_op = tf.reduce_sum(tf.square((self.q_out_op - self.ph_target_qs)))

    def _set_update_op(self):
        print self.loss_op
        grads_and_vars =self._opt.optimizer.compute_gradients(self.loss_op)
    ...
    def calc_target_q_values(self,  batched_transitions):
        ...
        _, targets = self.calc_forward(states)
        for i, x in enumerate(zip(batched_transitions)):
            state, action, reward, next_state, done = x[0]
            y, _ = self.calc_forward(state)
            _, next_y = self.calc_forward(next_state)
            n_action = np.nanargmax(next_y[0])
            max_next_y = next_y[0][n_action]
    ...
    def update_q_net(self, batched_transitions):
        ...
        return self._sess.run([self.loss_op, self.update_q_net_op], feed_dict=feed_dict) 

    def predict(self, batched_inputs, params, **kwargs):
        ys, _ = self.calc_forward(batched_inputs)
        return np.argmax(ys)
